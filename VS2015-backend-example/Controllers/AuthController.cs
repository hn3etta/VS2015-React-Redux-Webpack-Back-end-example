...
namespace BackendStarter.Controllers
{
	...
	public class AuthController : Controller
	{
		...
		[AllowAnonymous]
		[EnableCors("AllowAll")]
		public async Task<IActionResult> Get([FromBody] AuthUser applicationUser)
		{
		...
		/// </summary>
		private static Task<ClaimsIdentity> GetClaimsIdentity(AuthUser user)
		{
			if (user.UserName == "test@test.com" && user.Password == "Password")
			{
				return Task.FromResult(
					new ClaimsIdentity(new GenericIdentity(user.UserName, "Token"),
					new[] { new Claim("BasicAPI", "NormalUser") })
				);
			}
